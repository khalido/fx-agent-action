...
- **A per-key budget as the real control.** claude-code-action's cost advice is "set `--max-turns` and workflow timeouts"; Copilot bills a flat premium request; nobody else names a hard dollar ceiling enforced by the provider. fx's `vercel ai-gateway api-keys create --budget 20` plus "`max_cost` only notices afterwards" is more honest and more effective.
